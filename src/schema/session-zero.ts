...
//            design/14-glossary.md §Session Zero, §Tone, §Lethality, §Content Lines, §Veil
//            design/13-risks-tripwires.md §14 (session-zero state read by every agent at every scene)
//            design/11-dm-styles-tones.md §3 Tonal registers, §5 Safety / consent tooling
//
...
// above everything else) — see design/02-tools-orchestration.md §Context budgeting.
import { z } from 'zod';
...
// Cross-ref: design/14-glossary.md §Content Lines
...
export const ContentLineSchema = z.object({
  ...
  // Cross-ref: design/08-cross-cutting.md §Session zero §Open (how content lines are enforced)
}).strict();
export type ContentLine = z.infer<typeof ContentLineSchema>;

// ---------------------------------------------------------------------------
// Veil — content that may appear but always happens off-screen
// Cross-ref: design/14-glossary.md §Veil
// ---------------------------------------------------------------------------
...
// Cross-ref: design/08-cross-cutting.md §Session zero (explicit field list)
...
// context budget (design/02-tools-orchestration.md §Context budgeting).
// ---------------------------------------------------------------------------
export const SessionZeroSchema = z.object({
  ...
  // Cross-ref: design/14-glossary.md §Tone; design/11-dm-styles-tones.md §3
  tone: ToneSchema,
  ...
  // Cross-ref: design/14-glossary.md §Lethality
  ...
  // Cross-ref: design/11-dm-styles-tones.md §5 Safety / consent tooling
