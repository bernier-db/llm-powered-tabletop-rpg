...
Target ruleset: **Pathfinder-lightweight** (see `design/03-rules-combat.md`).

## Where the design lives

All design documents are in `design/`. Start with **`design/README.md`** for the index.

When you start a session, the typical pattern is:
1. Read `design/README.md` to orient
2. Check `design/TODO-BRAINSTORM.md` for explicitly-flagged open threads
3. Check `design/OPEN-QUESTIONS.md` for pending decisions
...
## Frontmatter convention (all design files)

Every file under `design/` carries YAML frontmatter:
...
related: [list of file paths within design/ or ..]
updated: YYYY-MM-DD
---
```
...
2. **Cross-file filtering** — `grep -l "status: AGREED" design/*.md` returns settled docs vs. in-progress.
3. **Cross-reference following** — the `related:` field encodes the local doc graph; load neighbors as needed.

When you edit a file, update its `updated` field. When you add a new design file, give it frontmatter and add it to `design/README.md`.
...
- **INDEX** — a navigation/index file (e.g., `design/README.md`)
...
- New threads/topics → add an entry to `design/TODO-BRAINSTORM.md` so future sessions know they exist

## Legacy

The original single-file brainstorm is preserved at `BRAINSTORM.md` (now a redirect). All content has been split into `design/`.
